# -*- coding: utf-8 -*-
"""M0 核验：用真模板重跑提取，与仓库内结构快照逐项比对（全表逐行），并落一份脱敏结构元信息。
...
- 只读模板；`--extra-out` 落盘的只有结构元信息（脚注原文/数据有效性/批注/隐藏行列/序号预填），
  不含业务数据。
- 防误传：全表逐格记账——非空单元格只允许出现在「表头行及其以上」「脚注行」「A 列纯数字序号预填」
  三处；出现其他内容即判定疑似真实数据，拒绝落盘并非零退出。
- 差异数 > 0 时非零退出。
"""
import argparse
import json
...
def extract_full(path):
    """真模板全表提取，与 scripts/m0_extract_struct.py 同口径（抓取范围=末行）。"""
...
def extract_extra(path, sheet_struct):
    """结构定性：表头行、脚注行、数据区空白性、下拉、批注、隐藏行列、序号预填。

    表头行取「脚注行之前最后一个含 2 个以上非空格子的行」（台账式=第 3 行，表单式蓄电池=第 4 行）。
    数据区 = 表头行+1 … 脚注行-1，除 A 列序号预填外应为全空。
    """
    ...
    header_row = None
    for row in ws.iter_rows(min_row=1, max_row=(footer_row or ws.max_row) - 1, max_col=ws.max_column):
        if sum(1 for c in row if cellstr(c.value)) >= 2:
    ...
    data_from = (header_row + 1) if header_row else None
    data_to = (footer_row - 1) if footer_row else None
    extra = {
        "sheet": ws.title,
        "max_row": ws.max_row,
        "max_col": ws.max_column,
        ...
        "data_region": {"from": data_from, "to": data_to, "unexpected_cells": unexpected},
        "data_validations": [
...
def compare(snapshot, fresh):
    """全表逐项比对：sheet 名 / dims / 行列数 / 合并单元格 / 每一行逐格。"""
    diffs = []
    ...
    if len(snapshot["rows"]) != fresh["max_row"]:
        diffs.append(f"快照行数 {len(snapshot['rows'])} != 末行 {fresh['max_row']}（抓取范围应覆盖全表）")
    for snap_row, fresh_row in zip(snapshot["rows"], fresh["rows"]):
        if snap_row["r"] != fresh_row["r"] or snap_row["vals"] != fresh_row["vals"]:
            diffs.append(f"第 {snap_row['r']} 行不一致")
    return diffs
...
def main(argv=None):
    ...
    for record_type, filename in TEMPLATES:
        ...
        if not snap:
            ...
            continue
        diffs = compare(snap["sheets"][0], sheet_struct)
        all_diffs.extend(f"{record_type}: {d}" for d in diffs)
        extra = extras[record_type]
        print(f"{record_type:<42} 全表 {len(sheet_struct['rows'])} 行逐格"
              f" {'一致' if not diffs else '★有差异'}"
              f"  表头 r{extra['header_row']} 脚注 r{extra['footer_row']}"
              f" 数据区 {extra['data_region']['from']}-{extra['data_region']['to']}"
              f" 异常内容 {len(extra['data_region']['unexpected_cells'])} 项"
    ...
    if args.extra_out:
        leaked = [
            ...
            for cell in extra["data_region"]["unexpected_cells"]
        ]
        if leaked:
            print("\n拒绝落盘：数据区检出非「表头/脚注/序号预填」的内容（疑似真实数据），请人工确认后再提交：")
            for item in leaked[:20]:
                print("  -", item)
            return 3
        payload = {
            "note": "脱敏结构元信息：脚注原文/数据有效性/批注/隐藏行列/序号预填/数据区空白性核对；不含业务数据",
