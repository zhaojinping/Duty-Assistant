...
name: duty-assistant
description: >
  蓄电池电压测量和设备测温的安装与日常运行。
  触发词：安装值班助手、蓄电池电压安装、电压测量安装、设备测温、Duty-Assistant 安装。
  读者是智能体。用户不懂命令。命令由你执行。所有智能体走同一份步骤。
---

# 蓄电池电压测量与设备测温 · 安装

本次交付蓄电池电压和设备测温。跳闸、避雷器、两票等其余记录不在这次安装里。用户问起就说明这次没有。
...
5. 班长姓名。可以不填。不填就不发私聊。危急测温也发给这个人。
6. 测温待办人。可以不填。不填就用上面的电压待办人。
7. 有几组电池，每组是 2V 还是 12V，多少只。不要用任何场站的现成四组。
8. 电压到期默认每月 15 日。告诉用户可以改日子，或改成每 30 天。没改就用每月 15 日，起算日用下一个 15 日。设备测温固定每月 10 日；每年 7–9 月改为上次测温后再过 7 天，10 月恢复到 10 日。不要改这套测温节奏，除非用户明确要求。
9. 合格范围先用 2V 为 1.85–2.35、12V 为 11.85–13.80。测温用热点超过 80℃ 或相对温差达到 80% 为严重，热点超过 110℃ 或相对温差达到 95% 为危急，相间温差超过 15K 为一般。用户不提出新数字就不改。
10. 录入页：按仓库里的 7 行文本（组别、温度、直流系统、浮充电压、测试性质、数据、提交时间）帮用户在 WorkBuddy 免费托管上新建自己的页面。测温可以走同一个拉取地址，报文里带 `record_type` 为 `infrared_thermography_record` 和 `spots`。必须本人点发布时，停下来等网址。网址必须是 https，且不能是 `battery-voltage-entry.app.workbuddy.host`。
...
班长有名字时再加 `--escalate-name` 和 `--escalate-id`。测温待办人另有其人时再加 `--thermo-assignee-name` 和 `--thermo-assignee-id`。
...
电压表建好后，在同一张钉钉表里加测温表。先说明将新建《设备测温记录》18 列，同意后再执行：

```text
python -m da_core.cli create-thermo-table --confirm
```

## 设备测温怎么交

群里先发下面这种文本，紧接着由同一个人按测点顺序发红外图。图片要在下一次拉取（约 5 分钟）之前发完。缺图会入库并提醒，不退回。
...
必填是设备名称、测点部位、实测温度、仪器编号，再加上环境温度。相间温差、δt、人工判级、致热类型可以不写。不写致热类型就按电流致热。人工判级只供核对，严重和危急以引擎算出的等级为准。严重及以上给测温待办人发待办，危急再私聊班长。一般只写在群回执里。
